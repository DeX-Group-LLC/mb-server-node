...
describe('MessageBroker', () => {
    ...
    describe('constructor', () => {
        ...
        it('should initialize all components in correct order', () => {
            ...
            expect(logger.info).toHaveBeenCalledWith(expect.stringContaining('[MessageBroker] Listening on'));
        ...
        it('should log server start with correct host and port', () => {
            ...
            expect(logger.info).toHaveBeenCalledWith(expect.stringMatching(/\[MessageBroker\] Listening on .+:\d+ \(WebSocket\)/));
        });
    });
    ...
    describe('shutdown', () => {
        ...
        it('should handle WebSocket server close error', async () => {
            ...
            expect(logger.error).toHaveBeenCalledWith('[MessageBroker] Error closing WebSocket server:', error);
        });
        ...
        it('should log shutdown steps', async () => {
            ...
            expect(logger.info).toHaveBeenCalledWith('[MessageBroker] Shutting down...');
            expect(logger.info).toHaveBeenCalledWith('[MessageBroker] WebSocket server closed');
            expect(logger.info).toHaveBeenCalledWith('[MessageBroker] Shutdown complete.');
        });
    });
});